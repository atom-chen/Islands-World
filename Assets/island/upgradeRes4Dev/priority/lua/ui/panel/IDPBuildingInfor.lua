...
---@class IDPBuildingInfor:IDBasePanel
local IDPBuildingInfor = class("IDPBuildingInfor", IDBasePanel)
...
function IDPBuildingInfor:init(csObj)
    ...
    IDPBuildingInfor.super.init(self, csObj)
    --[[
    ...
    if buildingAttrRoot == nil then
        CLUIOtherObjPool.borrowObjAsyn(
            ...
            function(name, go, orgs)
                ...
                SetActive(go, true)
                buildingAttrRoot:init(
                    {attr = attr, serverData = serverData, type = IDConst.AttrType.building, maxRow = 4},
                    nil
                )
                if callback then
                    callback()
