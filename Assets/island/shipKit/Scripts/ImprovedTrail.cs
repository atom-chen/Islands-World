...
public class ImprovedTrail : MonoBehaviour
{
    ...
    GameObject mChild
    {
        ...
        {
            if (_mChild == null) {
                _mChild = new GameObject(GetType() + " for " + name);
                mCamera = Camera.main.transform;
                mFilter = _mChild.AddComponent<MeshFilter>();
                mRen = _mChild.AddComponent<MeshRenderer>();
                mRen.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
                mRen.receiveShadows = false;
                mMesh = new Mesh();

#if UNITY_EDITOR
                _mChild.AddComponent<Coolape.ResetMat>();
#endif
	...
	void OnDisable () { 
        if (_mChild != null) mChild.active = false; 
