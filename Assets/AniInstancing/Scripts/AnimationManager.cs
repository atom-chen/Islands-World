...
{
	public class AnimationManager : MonoBehaviour// Singleton<AnimationManager>
	{
...
//#endif
        ...
        private IEnumerator LoadAnimationInfoFromAssetBundle(CreateAnimationRequest request)
        {
			...
			AssetBundleRequest abRequest = m_mainBundle.LoadAssetAsync(request.cfgName);
            yield return abRequest;

            bool find = false;
            InstanceAnimationInfo info = null;
			if (m_animationInfo.TryGetValue(request.cfgName, out info))
            {
                find = true;
			...
			if (abRequest != null && !find)
            {
				...
				AnimationInstancingMgr.Instance.ImportAnimationTexture(request.cfgName, reader);
                request.instance.Prepare(info.listAniInfo, info.extraBoneInfo);
				m_animationInfo.Add(request.cfgName, info);
            }
        }
