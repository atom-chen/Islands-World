...
namespace AnimationInstancing
{
	...
	public class AnimationInstancingMgr : MonoBehaviour//Singleton<AnimationInstancingMgr>
    {
		public static AnimationInstancingMgr Instance;
    ...
    public struct InstancingData 
    {
        ...
        {
			public Camera camera;		//add by chenbin
			public int layer;		//add by chenbin
			public ShadowCastingMode shadowCastingMode = ShadowCastingMode.On; // add by chenbin
			public bool receiveShadows = true; // add by chenbin

			public string nameCode; // modify by chenbin
...
//        public int currentInstancingIndex = 0;
        ...
        public List<AnimationInstancing> aniInstancingList;
        // to calculate lod level
        private Transform cameraTransform; 
		private Dictionary<string, VertexCache> vertexCachePool;  // modify by chenbin
		private Dictionary<string, InstanceData> instanceDataPool; // modify by chenbin
...
//			if(SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.OpenGLES3)
		...
		public static bool canUseInstancing()
		{
        ...
        private void Start()
        {
			vertexCachePool = new Dictionary<string, VertexCache>();
			instanceDataPool = new Dictionary<string, InstanceData>();
        }

		 public void InitializeCullingGroup(Camera camera = null)
        {
			if (cullingGroup != null) {
				cullingGroup.Dispose ();
			}
            cullingGroup = new CullingGroup();
			cullingGroup.targetCamera = camera == null ? Camera.main : camera;
            ...
            foreach (var obj in vertexCachePool)
			{
                ...
                for (int k = 0; k != vertexCache.packageList.Length; ++k)
				{
                    for (int i = 0; i != vertexCache.packageList[k].Count; ++i)
                    {
                        ...
                        for (int j = 0; j != package.subMeshCount; ++j)
                        {
                            if (useInstancing)
                            {
                                ...
                                Graphics.DrawMeshInstanced(vertexCache.mesh,
									...
									vertexCache.shadowCastingMode, // add by chenbin
									vertexCache.receiveShadows, // add by chenbin
									vertexCache.layer, // add by chenbin
									vertexCache.camera // add by chenbin
								);
                            }
                            else
                            {
                        ...
                        package.instancingCount = 0;		//modify by chenbin
                    }
...
//            script.prototype = prefab;
        ...
        public void AddInstance(GameObject obj)
        {
            ...
            try
            {
				...
				if (success) {
					if(!aniInstancingList.Contains(script)) {
		...
		public bool isInCameraView(Camera cam, Bounds bounds) {
        ...
        void ApplyBoneMatrix()
        {
            ...
            for (int k = 0, i =0; k != aniInstancingList.Count; ++k)
            {
                AnimationInstancing instance = aniInstancingList[k];
				...
				if (!instance.gameObject.activeInHierarchy || (!instance.visible && !isInCameraView (cullingGroup.targetCamera, instance.bounds ()))) { // modify by chenbin
					continue;
				}
				...
				//                int count = pkg.instancingCount - 1;		// del by chenbin
				Matrix4x4[] arrayMat = data.worldMatrix[aniTextureIndex][index];
				int count = pkg.instancingCount;
				if (count >= 0 && count < arrayMat.Length)
                {
                ...
                for (int j = 0; j != lod.vertexCacheList.Length; ++j)
                {
                    ...
                    if (package.instancingCount + 1 > instancingPackageSize)
                    {
                        ...
                        {
							VertexCache.InstancingPackage newPackage = CreatePackage(
								cache.instanceData,
                                cache.mesh,
		...
		public VertexCache FindVertexCache(string renderName)		//modify chenbin
        {
		...
		public VertexCache.InstancingPackage CreatePackage(InstanceData data, Mesh mesh, Material[] originalMaterial, int index)
        {
            ...
            {
#if USE_CONSTANT_BUFFER
//                package.material[i] = new Material(originalMaterial[i]);		// del by chenbin
				package.material[i] = originalMaterial[i];		// add by chenbin
		...
		#region add by chenbin
		public void RemoveInstancingPackage(AnimationInstancing instancing, ref VertexCache vertexCache) {
			#if USE_CONSTANT_BUFFER
			for (int k = 0; k != vertexCache.packageList.Length; ++k)
			{
				for (int i = vertexCache.packageList[k].Count-1; i >=0; i--)
				{
					VertexCache.InstancingPackage package = vertexCache.packageList[k][i];
					package.instancingCount--;
				}
		...
		public void RemoveMeshVertex(AnimationInstancing instancing,  AnimationInstancing.LodInfo[] lodInfo) {
			for (int x = 0; x < lodInfo.Length; ++x) {
				...
				for (int i = 0; i != lod.skinnedMeshRenderer.Length; ++i) {
					...
					string nameCode = getVertexName(lod.skinnedMeshRenderer [i]);
					VertexCache cache = null;
					if (vertexCachePool.TryGetValue (nameCode, out cache)) {
						cache.instancingCount--;
						RemoveInstancingPackage (instancing, ref cache);
						if(cache.instancingCount <= 0) {
//							int instanceName = prefabName.GetHashCode() + 0;
							InstanceData insd = null;
							instanceDataPool.TryGetValue(nameCode, out insd);
							instanceDataPool.Remove (nameCode);
							if (insd != null) {
								for(int ii = 0; ii < insd.worldMatrix.Length; ii++) {
									insd.worldMatrix[ii].Clear();
								}
								insd.worldMatrix = null;
								for(int ii = 0; ii < insd.frameIndex.Length; ii++) {
									insd.frameIndex[ii].Clear();
								}
								insd.frameIndex = null;
								insd = null;
							}
							//=================================
							VertexCache vc =  FindVertexCache (nameCode);
							vertexCachePool.Remove (nameCode);
							if(vc != null) {
								vc.mesh = null;
								vc.materials = null;
								vc.camera = null;
				...
				for(int i =0; i < lod.meshRenderer.Length; i++) {
					string nameCode = getVertexName(lod.meshRenderer [i]);
					VertexCache cache = null;
					if (vertexCachePool.TryGetValue (nameCode, out cache)) {
						cache.instancingCount--;
						RemoveInstancingPackage (instancing, ref cache);
...
//							int instanceName = prefabName.GetHashCode() + 0;
		...
		//add by chenbin
		public static string getVertexName(Renderer render, int alias = 0) {
			if (render == null)
				return "";
			string name = render.name.GetHashCode ().ToString();
			if (render.sharedMaterial != null) {
				name = name + "_" + render.sharedMaterial.name.GetHashCode ();
			}
			return name + "_" + alias;
		}
		#endregion
		...
		public void AddMeshVertex(AnimationInstancing instancing, 
            ...
            string alias = null)
        {
			string prefabName = instancing.cfgName; // add by chenbin
			#if UNITY_EDITOR
...
//#if !USE_CONSTANT_BUFFER		// del by chenbin
                        ++cache.instancingCount;
//#endif		// del by chenbin
                        continue;
                    }

					VertexCache vertexCache = CreateVertexCache(instancing, prefabName, nameCode, 0, m);
                    vertexCache.bindPose = bindPose.ToArray();
                    lod.vertexCacheList[i] = vertexCache;
					SetupVertexCache(instancing, vertexCache, lod.skinnedMeshRenderer[i], bones, bonePerVertex);
                }
                ...
                for (int i = 0, j = lod.skinnedMeshRenderer.Length; i != lod.meshRenderer.Length; ++i, ++j)
                {
					...
					string renderName = getVertexName(lod.meshRenderer[i], aliasName);
                    VertexCache cache = null;
                    if (vertexCachePool.TryGetValue(renderName, out cache))
                    { 
...
//#endif		//del by chenbin
					...
					VertexCache vertexCache = CreateVertexCache(instancing, prefabName, renderName, aliasName, m);
                    if (bindPose != null)
                        vertexCache.bindPose = bindPose.ToArray();
                    lod.vertexCacheList[lod.skinnedMeshRenderer.Length + i] = vertexCache;
					SetupVertexCache(instancing, vertexCache, lod.meshRenderer[i], m, bones, bonePerVertex);
                }
#if !USE_CONSTANT_BUFFER
            foreach (var obj in vertexCachePool)
            {
		...
		private VertexCache CreateVertexCache(AnimationInstancing instacing, string prefabName, string renderName, int alias, Mesh mesh)
        {
			VertexCache vertexCache = new VertexCache();
			//			int cacheName = renderName + alias;
			string cacheName = renderName;		// modify by chenbin
			vertexCachePool[cacheName] = vertexCache;
			vertexCache.camera = instacing.camera; // add by chenbin
			vertexCache.layer = instacing.layer; // add by chenbin
			if (vertexCache.layer < 0 || vertexCache.layer > 31) {
				vertexCache.layer = 0;
				Debug.LogError ("CreateVertexCache::DrawMesh layer has to be from in [0..31] range!:::" + instacing.name + "==layerMask==" + instacing.layer);
			}
			vertexCache.shadowCastingMode = instacing.shadowCastingMode; // add by chenbin
			vertexCache.receiveShadows = instacing.receiveShadows; // add by chenbin
            vertexCache.nameCode = cacheName;
            vertexCache.mesh = mesh;
			vertexCache.instancingCount = 1;
...
//            int instanceName = prefabName.GetHashCode() + alias;
			string instanceName = renderName; // modify by chenbin
		...
		private void SetupVertexCache(AnimationInstancing instancing, VertexCache vertexCache,
...
//				VertexCache.InstancingPackage package = CreatePackage(vertexCache.instanceData, vertexCache.mesh, render.materials, i);
        ...
        for (int j = 0; j != mesh.vertexCount; ++j)
        {
			...
			if (vertexCache.materials == null) {
//				vertexCache.materials = render.materials;		//del by chenbin
				vertexCache.materials = render.sharedMaterials;
			}
		...
		public void setCamera(Camera camera, int layer = 0) {
			if (cullingGroup != null) {
			...
			for (int  i = 0; i< aniInstancingList.Count; i++) {
				aniInstancingList [i].camera  = camera;
				aniInstancingList [i].layer = layer;
			}
		}
