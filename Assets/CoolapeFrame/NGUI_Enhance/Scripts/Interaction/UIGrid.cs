...
public class UIGrid : UIWidgetContainer
{
	...
	[HideInInspector][SerializeField] public bool sorted = false; // modify by chenbin
	...
	public virtual void Start () // modify by chenbin
	{
		if (!mInitDone) Init();
	...
	[HideInInspector]
	public Vector3 oldParentPos = Vector3.zero;
	[HideInInspector]
	public Vector2 oldParentClipOffset = Vector4.zero;
	UIPanel parent = null;
	bool hadGetParent = false;
	
	public void Awake ()
	{
		if (!hadGetParent) {
			hadGetParent = true;
			oldParentPos = transform.parent.localPosition;
			parent = transform.parent.GetComponent<UIPanel> ();
			if (parent != null)
				oldParentClipOffset = parent.clipOffset;
		}
	}
	
	public void changeOldClip(Vector3 pos, Vector2 clipOffset) {
		oldParentPos = pos;
		oldParentClipOffset = clipOffset;
	}
	public void resetPosition ()
	{
		if (parent != null) {
			parent.transform.localPosition = oldParentPos;
			parent.clipOffset = oldParentClipOffset;
		}
	}
