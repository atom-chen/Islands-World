...
public class UIPopupList : UIWidgetContainer
{
	...
	public List<string> valueItems = new List<string>();	// add by chenbin
	...
	public string value
	{
		...
		set
		{
			#region add by chenbin
			if(valueItems.Count >= items.Count) {
				int index = valueItems.IndexOf(value);
				if(index >= 0) {
					mSelectedItem = items[index];
				} else {
					mSelectedItem = value;
				}
			} else {
			...
			#endregion
//			mSelectedItem = value;		//del by chenbin
	...
	protected void TriggerCallbacks ()
	{
		if (current != this)
		{
			...
			if (mLegacyEvent != null) mLegacyEvent(value);		// modify by chenbin
			...
			else if (eventReceiver != null && !string.IsNullOrEmpty(functionName))
			{
				...
				eventReceiver.SendMessage(functionName, value, SendMessageOptions.DontRequireReceiver); //// modify by chenbin
			}
			current = old;
		}
	}
	...
	void onGetBackgroundSprite(params object[] paras) {
		if(!string.IsNullOrEmpty(highlightSprite)) {
			Callback cb = onGetHightlightSprite;
			atlas.borrowSpriteByname(highlightSprite, null, cb);
		} else {
			doOnClick();
		}
	}
	void onGetHightlightSprite(params object[] paras) {
		doOnClick();
	}
	#endregion

	void doOnClick() // modify by chenbin
	{
	...
	#region add by chenbin
	public void refreshItems(ArrayList _items, ArrayList _itemValues) {
		items.Clear();
		...
		for(int i =0; i < _itemValues.Count; i++) {
			valueItems.Add(_itemValues[i].ToString());
		}
	}
	#endregion
}
