...
public class UIPopupListInspector : UIWidgetContainerEditor
{
	...
	void OnSelectAtlas (Object obj)
	{
		...
		mList.atlasName = mList.atlas != null ? mList.atlas.name : "";		//add by chenbin
	...
	void OnBitmapFont (Object obj)
	{
		...
		mList.fontName = obj != null ? obj.name : "";	//add by chenbin
	...
	public override void OnInspectorGUI ()
	{
		...
		showValueItems ();	// add by chenbin

		GUI.changed = false;
		string sel = NGUIEditorTools.DrawList("Default", mList.items.ToArray(), mList.selectedItem);	// modify by chenbin
	...
	void DrawAtlas()
	{
		if (NGUIEditorTools.DrawHeader("Atlas"))
		{
			...
			NGUIEditorTools.DrawProperty("Atlas Name", serializedObject, "atlasName");	//add by chenbin
	...
	void showValueItems() {
		...
		string text = "";
		foreach (string s in mList.valueItems) text += s + "\n";
		...
		if (modified != text)
		{
			RegisterUndo();
			string[] split = modified.Split(new char[] { '\n' }, System.StringSplitOptions.RemoveEmptyEntries);
			mList.valueItems.Clear();
			foreach (string s in split) mList.valueItems.Add(s);
