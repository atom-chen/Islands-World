				...
				if (newName != spriteName)
				{
					...
					if (GUILayout.Button("Rename", GUILayout.Width(60f)))
					{
						...
						if (sprite != null)
						{
							...
							if(NGUISettings.atlas.isBorrowSpriteMode) {
								int index = (int) (NGUISettings.atlas.spriteMap [sprite.name]);
								NGUISettings.atlas.spriteMap.Remove(sprite.name);
								NGUISettings.atlas.spriteMap [newName] = index;
							}
							#endregion
							sprite.name = newName;
