...
public class EventDelegate
{
	...
	// Private variables
	public delegate void Callback(GameObject go); // modify by chenbin
	...
	public bool Execute (GameObject go) // modify by chenbin
	{
#if !REFLECTION_SUPPORT
		if (isValid)
		{
...
// #endif
				if (objs != null && objs.Length > 0) mCachedCallback(go);
			}
#endif
			return true;
		}
...
//			Debug.LogError("len==" + mParameters[0] + "==" + mParameters[0].value);
			if (len == 1 && mParameters[0].value == null)
			{
//				Debug.Log("################");
				object[] args = {go}; // add by chenbin
				mMethod.Invoke(mTarget, args); // modify by chenbin
			}
			else
			{
				...
				for (int i = 0, imax = mParameters.Length; i < imax; ++i)
				{
					if(i == 0 &&  mParameters[0].value == null) {  // add by chenbin
						mArgs[i] = go; // add by chenbin
					} else {
	...
	static public void Execute (List<EventDelegate> list, GameObject go) // modify by chenbin
	{
		if (list != null)
		{
